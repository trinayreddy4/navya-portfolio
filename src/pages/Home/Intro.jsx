...
const Intro = () => {
  return (
    <section
      ...
      <p className="text-sm uppercase tracking-[0.6rem] text-secondary">
        Portfolio 2025
      </p>
      <h1 className="mt-6 text-6xl font-semibold text-white sm:text-3xl">
        Hi, I&apos;m{" "}
        <span className="text-secondary">Navya Devi Sushma Sri Katamsetti</span>
      </h1>
      <h2 className="mt-4 max-w-2xl text-3xl font-medium text-tertiary sm:text-xl">
        Full-stack engineer focused on resilient backend systems, clean APIs,
        and delightful user experiences.
      </h2>
      <p className="mt-6 max-w-2xl text-lg text-tertiary sm:text-base">
        I&apos;m a B.Tech Computer Science & Engineering student at KL
        University with hands-on experience in Java, Spring Boot, React, and
        AWS. I enjoy building scalable academic platforms, optimizing data
        workflows, and delivering reliable, testable APIs for real users.
      </p>
      ...
      <div className="mt-14 grid grid-cols-3 gap-6 sm:grid-cols-1">
        {stats.map((item) => (
          <div
            ...
            <p className="mt-2 text-sm uppercase tracking-[0.3rem] text-tertiary">
              {item.label}
            </p>
          </div>
        ))}
